...
/**
 * OmniFocus MCP Server - Enhanced Edition
 * Extended MCP server implementation with advanced OmniFocus features
...
function log(...args) {
    console.error('[OmniFocus Enhanced]', ...args);
}
...
// Embedded fallback implementations (keeping original ones and adding new ones)
function executeEmbeddedScript(scriptName, args = []) {
    ...
    switch (scriptName) {
        case 'add_task':
            ...
            // Build AppleScript with enhanced properties
            script = `
        ...
        case 'list_projects':
            const includeStats = args[0] === 'true';
            script = `
                tell application "OmniFocus"
                    tell default document
                        set activeProjects to every project whose status is active
                        
                        if (count of activeProjects) = 0 then
                            return "📁 No active projects"
                        else
                            set projectList to "📁 Active Projects (" & (count of activeProjects) & "):"
                            repeat with aProject in activeProjects
                                set projectName to name of aProject
                                ${includeStats ? `
                                set taskCount to count of (tasks of aProject whose completed is false)
                                set projectList to projectList & return & "• " & projectName & " (" & taskCount & " tasks)"
                                ` : `
                                set projectList to projectList & return & "• " & projectName
                                `}
                            end repeat
                            return projectList
                        end if
            ...
            script = `
                tell application "OmniFocus"
                    tell default document
                        ...
                        set deferredTasks to every flattened task whose defer date > currentTime and completed is false
                        
                        if (count of deferredTasks) = 0 then
                            return "⏸️ No deferred tasks"
                        else
                            set taskList to "⏸️ Deferred Tasks (" & (count of deferredTasks) & "):"
                            repeat with aTask in deferredTasks
                                set taskName to name of aTask
                                set deferDate to defer date of aTask
                                set taskList to taskList & return & "• " & taskName & " (available " & (deferDate as string) & ")"
                            end repeat
                            return taskList
                        end if
                    end tell
                ...
                tell application "OmniFocus"
                    tell default document
                        ...
                        if (count of flaggedTasks) = 0 then
                            ...
                            set taskList to "🚩 Flagged Tasks (" & (count of flaggedTasks) & "):"
                            repeat with aTask in flaggedTasks
                                ...
                                on error
                                    set taskList to taskList & return & "• " & taskName & " [Inbox]"
                                end try
                            end repeat
                            return taskList
                        end if
                ...
                tell application "OmniFocus"
                    tell default document
                        ...
                        if (count of overdueTasks) = 0 then
                            ...
                            set taskList to "⚠️ Overdue Tasks (" & (count of overdueTasks) & "):"
                            repeat with aTask in overdueTasks
                                set taskName to name of aTask
                                set dueDate to due date of aTask
                                set daysPast to ((currentTime - dueDate) / days) as integer
                                
                                if daysPast = 0 then
                                    set overdueTxt to "today"
                                else if daysPast = 1 then
                                    set overdueTxt to "1 day"
                                else
                                    set overdueTxt to daysPast & " days"
                                end if
                                
                                set taskList to taskList & return & "• " & taskName & " (overdue " & overdueTxt & ")"
                            end repeat
                            return taskList
                        end if
                    end tell
        ...
        // Include original embedded scripts
        case 'list_inbox':
        case 'today_tasks':
        case 'complete_task':
        case 'weekly_review':
            // Use original embedded implementations from before
            return executeOriginalEmbeddedScript(scriptName, args);
...
// Keep original embedded scripts for backward compatibility
function executeOriginalEmbeddedScript(scriptName, args = []) {
    // Include the original embedded script implementations here
    // (copying from the original server/index.js)
    let script = '';
...
// Main server loop
log('Starting OmniFocus MCP Server (Enhanced Edition)...');
...
log('Server ready with enhanced features, waiting for requests...');
